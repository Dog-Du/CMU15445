...
void MyTableLockTest() {
  ...
  auto task = [&](int txn_id) {
    ...
    for (const table_oid_t &oid : oids) {
      try {
        res = lock_mgr.LockTable(txns[txn_id], LockManager::LockMode::EXCLUSIVE, oid);
        BUSTUB_ASSERT(!res, "res should be false\n");
      } catch (TransactionAbortException &e) {
        // printf("%s", e.GetInfo().c_str());
        BUSTUB_ASSERT(e.GetAbortReason() == AbortReason::LOCK_ON_SHRINKING, "lock on shrinking\n");
      } catch (const char *ch) {
        printf("%s", ch);
      }
...
TEST(LockManagerTest, MyTableLockTest) { MyTableLockTest(); }  // NOLINT

TEST(LockManagerTest, MyTableLockTest2) {
  LockManager lock_mgr{};
  TransactionManager txn_mgr{&lock_mgr};
...
void RowLockTest1() {
  ...
  auto task = [&](int txn_id) {
    ...
    if (!txns[txn_id]->IsRowSharedLocked(oid, rid)) {
      printf("%d do not have shared lock \n", txn_id);
    }
