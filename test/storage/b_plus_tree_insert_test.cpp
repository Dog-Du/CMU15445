...
TEST(BPlusTreeTests, MyInsertTest) {
  ...
  std::vector<RID> rids;
  for (auto key : keys) {
    std::cout << "checkvalue : " << key << std::endl;
    rids.clear();
    index_key.SetFromInteger(key);
    tree.GetValue(index_key, &rids);

    if (rids.size() != 1) {
      std::cout << "error in checkvalue" << key << std::endl;
      abort();
    }
  ...
  for (auto key : vec) {
    ...
    tmp.clear();
    index_key.SetFromInteger(key);
    if ((tree.GetValue(index_key, &tmp)) != (mp.count(key) > 0)) {
      std::cout << "error in getvalue : " << key << std::endl;
      abort();
    }
  }
  ...
  for (auto key : keys) {
    ...
    EXPECT_EQ(tree.GetValue(index_key, nullptr), (mp.count(key) > 0));
    if ((tree.GetValue(index_key, &tmp)) != (mp.count(key) > 0)) {
      std::cout << "error in remove : " << key << std::endl;
      abort();
    }
  }
