...
class ExtendibleHashTable : public HashTable<K, V> {
 public:
   ...
   * 用IndexOf(key)函数找到字典引索，
   * @param key The key to be searched.
   * @param[out] value The value associated with the key.、
   * @return True if the key is found, false otherwise.
   * 通过引用型的V返回找到的值，用bool表示查找是否成功。
   */
   ...
   *
   * 插入给定的键值对到哈希表中。
   * 如果key已经存在，则value应该被更新
   * 如果桶已满并且无法在被插入，做下列事情在再次尝试之前：
   *  1. 如果桶的local_depth等于global_depth
   *      增加global_depth，然后把字典大小翻倍
   *  2. 增加桶的local_depth
   *  3. 分裂桶并且重新分配在桶中的字典指针引用键值对。
   ...
   * 给定键，删除对应的在桶中的键值对。在这个项目中桶收缩和合并是不被要求的。
   * @param key The key to be deleted.
   * @return True if the key exists, false otherwise.
   * 如果key存在则true，否则为false
   */
 ...
 private:
  ...
  // and remove the ones you don't need.ddddd

  int global_depth_{0};  // The global depth of the directory
  size_t bucket_size_;   // The size of a bucket, 这应该是一个const值，不能修改，
